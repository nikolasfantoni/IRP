...
for (i in seqi){
  ...
  for (j in seqj){
    ...
    for (m in 1:length(c1)){
      pir <- length(c1[[m]][,1])/totalc1
      f1<- rbind(f1,pir*pdf2var(i,j,mean(c1[[m]][,1]),mean(c1[[m]][,2]),s1[[m]][1,1],s1[[m]][1,2],pc1[[m]]))
    }
    for (m in 1:length(c2)){
      pir <- length(c2[[m]][,1])/totalc2
      f2<- rbind(f2,pir*pdf2var(i,j,mean(c2[[m]][,1]),mean(c2[[m]][,2]),s2[[m]][1,1],s2[[m]][1,2],pc2[[m]]))
    }
    f1 <- sum(f1)
    f2 <- sum(f2)
    if (f2 == 0){
